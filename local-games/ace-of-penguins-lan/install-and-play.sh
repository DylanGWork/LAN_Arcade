...
echo "Installing ace-of-penguins from GannanNet..."
...
$SUDO apt-get -o Dir::Etc::sourcelist="$TEMP_DIR/sources.list" -o Dir::Etc::sourceparts="-" -o Dir::State::lists="$TEMP_DIR/lists" --no-install-recommends install -y "ace-of-penguins"
if [ "${LAN_ARCADE_INSTALL_ONLY:-0}" = "1" ]; then
  echo "Ace of Penguins installed successfully."
  exit 0
fi
echo "Starting Ace of Penguins..."
exec /usr/games/ace-freecell
