...
echo "Installing tuxmath from GannanNet..."
...
$SUDO apt-get -o Dir::Etc::sourcelist="$TEMP_DIR/sources.list" -o Dir::Etc::sourceparts="-" -o Dir::State::lists="$TEMP_DIR/lists" --no-install-recommends install -y "tuxmath"
if [ "${LAN_ARCADE_INSTALL_ONLY:-0}" = "1" ]; then
  echo "TuxMath installed successfully."
  exit 0
fi
echo "Starting TuxMath..."
exec /usr/games/tuxmath
