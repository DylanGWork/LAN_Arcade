...
STATUS_LABELS = {
    ...
    'source-ready': 'Ready in browser',
    'partial': 'Starts, needs testing',
    'blocked': 'Not ready',
    'candidate': 'Needs game files',
}
        ...
        with path.open(newline='', encoding='utf-8') as f:
            for row in csv.DictReader(f):
                ...
                out.append(dict(
                    ...
                    runtime=runtime or 'Browser classic PC emulator',
...
<p class="meta">Classic PC - {html.escape(g["genre"])} - {html.escape(g.get("runtime") or "Browser classic PC emulator")}</p>
...
</style></head><body><main><div class="top"><div><h1>Classic PC Games</h1><p>Classic computer games run here in the browser where possible. If a game says Game files needed, add the local game files before it can launch offline.</p></div><a href="../games/">Back to Arcade</a></div><div class="notice">Games with Play are ready to try in the browser. Games marked Game files needed are listed for future import and need local files before they can launch offline.</div><section class="stats"><div class="stat"><strong>{len(games)}</strong><span>games listed</span></div><div class="stat"><strong>{packaged}</strong><span>playable here</span></div><div class="stat"><strong>{stats['source-ready']}</strong><span>ready in browser</span></div><div class="stat"><strong>{stats['candidate']}</strong><span>need game files</span></div></section><section class="toolbar"><input id="q" type="search" placeholder="Search old PC games"><div class="filters"><button class="active" data-f="all">All</button><button data-f="smoke-pass">Play-tested</button><button data-f="source-ready">Ready in browser</button><button data-f="partial">Starts, needs testing</button><button data-f="blocked">Not ready</button><button data-f="candidate">Needs game files</button></div></section><section class="grid">{''.join(cards)}</section></main><script>
...
async function boot(){
  ...
  const statusLabels={'smoke-pass':'Play-tested','source-ready':'Ready in browser','partial':'Starts, needs testing','blocked':'Not ready','candidate':'Needs game files'};
